...
class IBVSRCController(Node):
    ...
    def cb_corners(self, msg):
        ...
        for i, p in enumerate(msg.polygon.points):
            ...
            pixel_err_list.append(u - ud)
            pixel_err_list.append(v - vd)

            x, y = (u - CX)/FX, (v - CY)/FY
            xd, yd = (self.desired_pts[i] - [CX, CY]) / [FX, FY]

            error_vector.append(x - xd)
            error_vector.append(y - yd)
            L_stack.append(self.interaction_matrix(u, v, Z))
        ...
        pixel_err_magnitude = np.abs(pixel_err_list)
...
#        err_msg.data = np.array(e, dtype=np.float32).tolist()
