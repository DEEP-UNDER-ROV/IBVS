    ...
    def build_interaction_matrix(self, state):
        ...
        for i in range(self.N):
            ...
            if self.use_3d_matrix_feature:
                L_norm = self.interaction_matrix_3d(x, y, Z)
                A = np.diag([FX, FY, 1.0])

            else:
                L_norm = self.interaction_matrix_2d(x, y, Z)
                A = np.diag([FX, FY])

            # -------------------------------------------------
            # Normalized feature velocity ->
            # pixel feature velocity
            #
            # u_dot = FX * x_dot
            # v_dot = FY * y_dot
            # Z_dot = Z_dot
...
if __name__ == "__main__":
    main()## Stereo-enhanced IBVS Ls 3x6 all normalized
